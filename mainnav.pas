...
type
  ...
  TfmMainNav3 = class(TForm)
    ...
    function edListFilterItemEx(const ACaption: string; ItemData: Pointer; out
      Done: Boolean): Boolean;
    function edListFilterNode(ItemNode: TTreeNode; out Done: Boolean): Boolean;
...
function TfmMainNav3.edListFilterItemEx(const ACaption: string; ItemData: Pointer; out Done: Boolean): Boolean;
begin
  Done := True;
  Result := False;
end;

function TfmMainNav3.edListFilterNode(ItemNode: TTreeNode; out Done: Boolean): Boolean;
//var
//  s: string;
begin
  //Done := True;
  //s := edList.Text;
  //Result := false;// (Pos(s, ItemNode.Text) <> -1) and (ItemNode.Level > 0);
end;
...
procedure TfmMainNav3.edPassChange(Sender: TObject);
const
  wrongLetters: string = 'абвгдеёжзийклмнопрстуфхцчшщъыьэюя';
var
  en: boolean;
  c: char;
begin
  en := edPass.Text <> '';
  if en then
    for c in wrongLetters do
      if Pos(c, AnsiLowerCase(edPass.Text)) > 0 then
      begin
        en := false;
        break;
      end;
  btOk.Enabled := en;
  if en or (edPass.Text = '') then
    edPass.Font.Color := clDefault
  else
    edPass.Font.Color := clRed;
end;
